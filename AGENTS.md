...
- Direct VS Code → Z.ai PaaS works without a proxy for `glm-5.3-flash`, `glm-5.3`, `glm-5.2`, `glm-5.1`, and `glm-5v-turbo`.
...
- Vision is supported on `glm-5.3-flash` (native multimodal) and `glm-5v-turbo`. `glm-5.3`, `glm-5.2`, and `glm-5.1` are text-only.
