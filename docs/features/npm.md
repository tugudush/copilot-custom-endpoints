...
- npm package names **cannot** contain colons (`:`), so `npx copilot-proxy:kimi` is not possible as a literal package name.
...
Publish a single package `copilot-proxy` with a subcommand-style CLI entry point.

```
npx copilot-proxy kimi    → starts the Kimi proxy on :3457
npx copilot-proxy qwen    → starts the Qwen proxy on :3458
```
...
| ----------------------------------------------------------------------- | --------------------------------------- | ---------------------------------------- |
| One package, subcommand CLI (`copilot-proxy kimi`)                      | Clean, single install                   | Medium — need a CLI wrapper              |
| Two packages (`copilot-proxy-kimi`, `copilot-proxy-qwen`)               | `npx copilot-proxy-kimi` works directly | Low — but two publish steps, two READMEs |
| One package, multiple bins (`copilot-proxy-kimi`, `copilot-proxy-qwen`) | `npx copilot-proxy-kimi` works directly | Medium — need shebangs + bin entries     |

**Decision: single package with subcommand CLI.** It's the cleanest UX. Also expose direct bins as aliases so `npx copilot-proxy-kimi` also works.
...
```json
{
  ...
  "type": "module",
  "bin": {
    "copilot-proxy": "cli.mjs",
    "copilot-proxy-kimi": "proxy/kimi-proxy.mjs",
    "copilot-proxy-qwen": "proxy/qwen-proxy.mjs"
  },
  "files": ["cli.mjs", "proxy/"],
  "scripts": {
    "proxy:kimi": "node proxy/kimi-proxy.mjs",
...
if (sub === 'kimi') await import('./proxy/kimi-proxy.mjs')
  ...
  console.error('Usage: copilot-proxy <kimi|qwen>')
  process.exit(1)
}
```
...
3. `npm publish`
4. Test: `npx copilot-proxy kimi --help`
...
| -------- | -------------------------- | ----------------------------------------- |
| Scope    | Unscoped — `copilot-proxy` | Simpler `npx` UX                          |
...
| 2026-06-01 | **Phase 2 done.** `cli.mjs` subcommand dispatcher created. `package.json` updated with `bin` entries (`copilot-proxy`, `copilot-proxy-kimi`, `copilot-proxy-qwen`) and `files` for publish. Validated: `node cli.mjs kimi --help`, `node cli.mjs qwen --help`, `node cli.mjs` (usage). |
| 2026-06-01 | **Phase 3 done.** README.md and AGENTS.md updated with `npm run` and `npx` usage.                                                                                                                                                                                                      |
| 2026-06-01 | **clean:logs added.** `npm run clean:logs` and `npx copilot-proxy clean` remove `debug_log/`.                                                                                                                                                                                          |
